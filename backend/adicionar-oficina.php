...
if (!in_array($_SESSION['usuario_tipo'] ?? 'funcionario', $permite, true)) {
    header('Location: ../oficinas.html?erro=sem_permissao');
    exit;
}
...
if ($nome === '' || !filter_var($email, FILTER_VALIDATE_EMAIL) || strlen($cep) !== 8 || !in_array(strlen($documento), [11, 14], true)) {
    header('Location: ../adicionar-oficina.html?erro=dados_invalidos');
    exit;
}
...
header('Location: ../oficinas.html?sucesso=oficina_criada');
exit;
